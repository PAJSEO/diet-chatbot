...
export default function AppetiteControlChatbot() {
    ...
    const fetchHistory = async (id) => {
        ...
        try {
            ...
            setMessages([
                { role: 'assistant', content: `${id}, 다시 왔군? 오늘도 쌈뽕하게 놀아보자구?` },
                ...history
            ]);
        } catch (error) {
            console.error("Failed to fetch history:", error);
            setMessages([{ role: 'assistant', content: '대화 기록을 불러오는데 실패했다냥 ㅠ' }]);
        } finally {
            setIsLoading(false);
        }
    };
